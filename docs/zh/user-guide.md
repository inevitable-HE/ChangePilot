...
创建变更时可填写一个 GitHub Pull Request URL。默认离线模式会保存该上下文，
但不会访问网络。启用 DeepSeek 在线模式后，模型可以调用
`github.inspect_pull_request`，读取 PR 标题、分支、变更文件和 CI Check。

控制台的“外部工具调用”区域显示实际调用与截断结果，“RAG 检索证据”显示
Runbook 依据。两类上下文共同进入规划，但都不能绕过最终的工具白名单、风险
策略和审批校验。

GitHub 集成只读，并且只接受 `https://github.com/.../pull/...`。它不会合并
PR、修改代码、触发工作流或执行发布。
